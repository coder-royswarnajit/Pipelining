...
def run_eda_pipeline(df, save_plots=True, plot_folder="eda_plots", target_column=None):
    ...
    if save_plots:
        ...
        box_plot_paths = box_plots(
            df,
            plot_folder=plot_folder
        )

        if box_plot_paths is not None:
            if isinstance(box_plot_paths, list):
                plot_paths.extend(box_plot_paths)
            else:
                plot_paths.append(box_plot_paths)
